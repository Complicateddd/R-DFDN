...
trainloader, validloader, testloader_cor, nb_classes, dim_inp = get_dataset(args)

args.dataset='mnist'
_, _, testloader_mnist, _, _ = get_dataset(args)
args.dataset='mnistm'
_, _, testloader_mnistm, _, _ = get_dataset(args)
args.dataset='svhn'
_, _, testloader_svhn, _, _ = get_dataset(args)
args.dataset='usps'
_, _, testloader_usps, _, _ = get_dataset(args)
...
train_loss_list, train_acc_list, valid_acc_list, test_acc_list_cor, reg_loss_list,test_acc_list_mnist,test_acc_list_mnist_m,test_acc_list_usps\
,test_acc_list_svhn = [], [], [], [], [],[],[],[],[]
epoch = 0
...
def train_fn():
    ...
    while epoch<args.epochs:
        ...
        test_acc_cor = test(testloader_cor, model=model, save=True, epoch=epoch)
        test_acc_mnist = test(testloader_mnist, model=model, save=False, epoch=epoch)
        test_acc_mnist_m = test(testloader_mnistm, model=model, save=False, epoch=epoch)
        test_acc_svhn = test(testloader_svhn, model=model, save=False, epoch=epoch)
        test_acc_usps = test(testloader_usps, model=model, save=False, epoch=epoch)


        test_acc_list_cor.append(test_acc_cor)
        test_acc_list_mnist.append(test_acc_mnist)
        test_acc_list_mnist_m.append(test_acc_mnist_m)
        test_acc_list_svhn.append(test_acc_svhn)
        test_acc_list_usps.append(test_acc_usps)
        ...
        with open(args.save_dir + "/test_acc_cor.pkl", "wb") as f:
            pkl.dump(test_acc_list_cor, f)
        with open(args.save_dir + "/test_acc_mnist.pkl", "wb") as f:
            pkl.dump(test_acc_list_mnist, f)
        with open(args.save_dir + "/test_acc_mnist_m.pkl", "wb") as f:
            pkl.dump(test_acc_list_mnist_m, f)
        with open(args.save_dir + "/test_acc_svhn.pkl", "wb") as f:
            pkl.dump(test_acc_list_svhn, f)
        with open(args.save_dir + "/test_acc_usps.pkl", "wb") as f:
            pkl.dump(test_acc_list_usps, f)

        with open(args.save_dir + "/reg_loss_list.pkl", "wb") as f:
            pkl.dump(reg_loss_list, f)


        status = 'Epoch {}/{} | Loss {:3.4f} | acc {:3.2f} | test-acc_cor {:3.2f} | test-acc_mnist {:3.2f} | test-acc_mnist_m {:3.2f} \
        | test-acc_svhn {:3.2f} |test-acc_usps {:3.2f} |reg_loss : {:3.4f}'.\
            format( epoch, args.epochs, loss, train_acc, test_acc_cor, test_acc_mnist,test_acc_mnist_m,test_acc_svhn,test_acc_usps,regularization_loss)
        print (status)
...
with open(args.save_dir + '/log.txt', 'a') as f:
        f.write(status + '\n')
print(max(test_acc_list_cor))
print(max(test_acc_list_mnist))
print(max(test_acc_list_mnist_m))
print(max(test_acc_list_svhn))
print(max(test_acc_list_usps))
